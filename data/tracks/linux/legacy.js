...
"ดิสก์เต็มเพราะ log" คือสาเหตุอันดับต้น ๆ ที่ทำให้เซิร์ฟเวอร์ล่มโดยไม่จำเป็น</div>`,
      ...
      labs: [{
        ...
        tasks: [
          ...
          { t: 'เริ่มและเปิดใช้งานอัตโนมัติของ <code>nginx</code>', hint: 'sudo systemctl enable nginx → sudo systemctl start nginx', check: s => s.services.nginx.active && s.services.nginx.enabled },
...
กำหนด <b>RPO</b> (ยอมเสียข้อมูลย้อนหลังได้กี่นาที) และ <b>RTO</b> (ยอมให้ล่มได้กี่นาที) ให้ชัด แล้วซ้อมกู้จริงอย่างน้อยปีละครั้ง พร้อมจับเวลาว่าทำได้ตาม RTO หรือไม่</div>`,
      ...
      labs: [{
        ...
        tasks: [
          ...
          { t: 'เปิดใช้งาน ufw', hint: 'sudo ufw enable', check: s => s.ufw.active },
          { t: 'สร้างไดเรกทอรี <code>/etc/sysctl.d</code> และไฟล์ <code>99-hardening.conf</code>', hint: 'sudo mkdir -p /etc/sysctl.d → echo "net.ipv4.tcp_syncookies = 1" > /etc/sysctl.d/99-hardening.conf', check: s => { const f = node(s, '/etc/sysctl.d/99-hardening.conf'); return f && f.content.length > 0; } },
