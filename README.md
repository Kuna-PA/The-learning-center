...
ทุก Lab มีนาฬิกาอยู่บนแผงด้านขวา และทำงานคนละแบบตามโหมด

| โหมด | นาฬิกา | เมื่อทำจบ |
|---|---|---|
| Lab ปกติ | **นับขึ้น** จากศูนย์ | สรุปเวลาที่ใช้ และบอกว่าทำลายสถิติเดิมของตัวเองไหม |
| เอาชีวิตรอด | **นับถอยหลัง** จากเวลาเป้าหมายของเหตุการณ์นั้น | บอกว่าทันเวลาเป้าหมายหรือไม่ |

นาฬิกานับถอยหลังเปลี่ยนสีเตือนก่อนหมดเวลา — **เหลือ 25% เป็นเหลือง · เหลือ 10% หรือ 30 วินาที
เป็นแดงและเต้นเป็นจังหวะ** (Lab สั้น ๆ 10% อาจเหลือแค่สิบกว่าวินาทีซึ่งสั้นเกินกว่าจะตั้งตัว
จึงมีเพดาน 30 วินาทีคุมอีกชั้น)

**หมดเวลาแล้วจะยังทำต่อได้** นาฬิกาเปลี่ยนเป็นนับเวลาที่เกินมาให้เห็น (💀 +2:14)
แต่**จูล่งจะกระโดดเข้ามาฟันหนึ่งที** แล้วขึ้นการ์ดว่า
*"เจ้าช่างอ่อนหัดยิ่งนัก กลับไปฝึกฝามาใหม่ซะ"* พร้อมปุ่ม **↺ เริ่มใหม่** กับ **ขอสู้ต่อ**

> ตั้งใจให้กดดันเรื่องเวลา ไม่ใช่ลงโทษ — ความคืบหน้าที่ทำไปแล้วไม่ถูกล้าง
> และเวลาที่บันทึกเป็นสถิติจะนับเฉพาะรอบที่ลงมือทำจริง ตอนกลับมาเปิด Lab เดิม
> แล้วระบบเล่นคำสั่งเก่าซ้ำให้ จะไม่ถูกนับเป็นเวลาใหม่

เวลาที่ทำได้ดีที่สุดของแต่ละ Lab เก็บไว้ในความคืบหน้าของผู้เรียน (`bestSeconds`)
และแสดงเป็นป้าย "สถิติของคุณ" ใต้ในนาฬิกาเมื่อกลับมาทำซ้ำ
...
assets/               รูปที่ใช้ในหน้าเว็บ (โลโก้ · จูล่ง) — สร้างด้วย scripts/make-assets.mjs
  devices/            ไอคอนอุปกรณ์รายตัว — ตัดจากแผ่นรวมด้วย scripts/split-device-icons.mjs
server/
...
js/
  ...
  timer.js            นาฬิกาจับเวลาของ Lab (นับขึ้น / นับถอยหลัง)
...
| เพิ่มอะไร | กรอกอะไร |
|---|---|
| **บทเรียน** | หัวข้อย่อย + เนื้อหา (ใส่ HTML ได้ เช่น `<p> <ul> <code>`) + ภาพประกอบกี่รูปก็ได้ |
| **ข้อสอบ** | เลือกตอบข้อเดียว / หลายข้อ / พิมพ์คำสั่ง + คำอธิบายเฉลย + ภาพประกอบ 1 รูป |
| **Lab** | ชื่อ · สถานการณ์ · อุปกรณ์จำลอง · ภาพประกอบ · ขั้นตอนทีละข้อ (สิ่งที่ต้องทำ + คำใบ้ + กติกาตรวจ) |

#### ภาพประกอบ

กดปุ่ม **🖼️ เลือกรูป** ในฟอร์มไหนก็ได้ — ระบบ**ย่อรูปให้เองในเบราว์เซอร์**ก่อนบันทึก
(กว้างไม่เกิน 1280px แปลงเป็น WebP) เพราะ screenshot จากจอ 4K หรือรูปจากมือถือ
มักหลายเมกะไบต์ ถ้าฝังดิบ ๆ เนื้อหาจะบวมจนโหลดช้าสำหรับผู้เรียนทุกคน

| ที่ | รูปไปโผล่ตรงไหน |
|---|---|
| บทเรียน | วางเองได้ด้วยโค้ด `[[รูป 1]]` ตรงตำแหน่งที่ต้องการ (กดปุ่ม "แทรกในเนื้อหา") — ไม่วางก็ไปต่อท้ายบทเรียนให้ |
| ข้อสอบ | ใต้คำถาม เหนือตัวเลือก — เช่นให้ดูผัง topology หรือ output แล้วตอบ |
| Lab | ใต้สถานการณ์ในหัวหน้า Lab — เช่นผังเครือข่ายของ Lab นั้น |

รูปถูก**ฝังไปกับเนื้อหาเลย** (data URI) ไม่ต้องมีที่เก็บไฟล์แยก จึงใช้ได้ทั้งบนเซิร์ฟเวอร์
และบน static hosting และยัง**เห็นรูปตอนออฟไลน์**ด้วย · เพดานคือ **900 KB ต่อรูป**
และ **4 MB สำหรับเนื้อหาทั้งหมด** (เผื่อโควตา localStorage ~5MB) เกินแล้วจะเตือนตอนบันทึก

ที่มาของรูปที่ระบบยอมรับมีสามแบบเท่านั้น — รูปที่อัปโหลดเข้ามา · ไฟล์ใน `assets/` ของ repo ·
ลิงก์ `https://` (ลิงก์ภายนอกจะไม่ขึ้นตอนออฟไลน์) ที่มาอื่นเช่น `javascript:` หรือ `http://`
จะถูกตัดทิ้งทั้งฝั่งเบราว์เซอร์และฝั่งเซิร์ฟเวอร์

> **เนื้อหาที่เขียนด้วยโค้ดก็ใส่รูปได้เหมือนกัน** — วางไฟล์ไว้ใน `assets/`
> แล้วอ้างใน `h:` ของ section ตรง ๆ เช่น `<img src="assets/vlan-diagram.png">`
...
| ไอคอนอุปกรณ์ | `assets/devices/*.png` | หัวหน้า Lab · ตารางอุปกรณ์ในบทเรียน · ข้อสอบแบบดูภาพ |

### ไอคอนอุปกรณ์ (15 ตัว)

ตัดจากแผ่นรวมใบเดียวออกมาเป็นไฟล์รายตัว เพื่อให้**หยิบไปใช้เฉพาะตัวที่ตรงกับงาน** —
Lab ของ Cisco ขึ้นรูปสวิตช์ Lab ของ MikroTik Router ขึ้นรูปเราเตอร์ ไม่ใช่แปะแผ่นรวมทั้งแผ่น
แต่ละไฟล์ตัดเฉพาะตัวการ์ด ไม่เอาป้ายชื่อ และทำมุมโค้งให้โปร่งใสเพื่อให้วางบนพื้นสีเข้มได้สวย
...
| ไฟล์ | ใช้ที่ไหน |
|---|---|
| `switch.png` | หัวหน้า Lab ของ `cisco` · `mikrotik-sw` · ตารางอุปกรณ์ · ข้อสอบ |
| `router.png` | หัวหน้า Lab ของ `mikrotik` · ตารางอุปกรณ์ · ข้อสอบ |
| `server.png` | หัวหน้า Lab ของ `linux` · `linux-sec` · `windows` · ข้อสอบ |
| `computer.png` | หัวหน้า Lab ของ `windows-gui` |
| `hub.png` · `l3-switch.png` · `firewall.png` · `access-point.png` · `wlc.png` · `load-balancer.png` · `proxy.png` · `ids-ips.png` · `media-converter.png` | ตารางอุปกรณ์ในบทเรียน Network และข้อสอบแบบดูภาพ |
| `notebook.png` · `transceiver.png` | ยังไม่ได้ใช้ — หยิบไปใส่บทเรียนหรือ Lab ใหม่ได้เลย |

แผนที่ว่าอุปกรณ์จำลองไหนใช้รูปไหนอยู่ที่ `DEVICE_ICONS` ใน [`js/devices/index.js`](js/devices/index.js)
เพิ่มอุปกรณ์จำลองใหม่แล้วอย่าลืมเพิ่มรูปให้ด้วย ไม่งั้นหัวหน้า Lab จะกลับไปใช้ 🧪 เหมือนเดิม

> ป้ายในแผ่นต้นฉบับมีสองจุดที่ต้องระวัง — ช่องที่ 3 เขียนว่า "cHub" (คือ hub)
> และสองช่องท้ายติดป้ายสลับกัน โดยช่อง "15. NATGNAL" อ่านไม่ออกว่าตั้งใจให้เป็นอะไร
> จึงตั้งชื่อไฟล์ตามสิ่งที่รูปสื่อไว้ก่อนเป็น `transceiver.png`
